...
try {
    for ($mask = $StartMask; $mask -lt 16; ++$mask) {
        ...
        $previous = Join-Path $outputs "VF2-Mobile-Furniture-With-Island-Events-B151-$variant"
        $out = Join-Path $outputs "VF2-Mobile-Furniture-With-Island-Events-B152-$variant"
        ...
        $patchLog = Join-Path $out "b152-patch.log"
        $buildLog = Join-Path $out "b152-build.log"
...
Write-Host "Running B152 linked Holiday positive/negative validation"
& $pythonExe @pythonArgs $validator
if ($LASTEXITCODE -ne 0) {
    throw "B152 linked Holiday validation failed with exit code $LASTEXITCODE"
}

Write-Host "B152 16-state executable matrix and linked validation completed successfully."
