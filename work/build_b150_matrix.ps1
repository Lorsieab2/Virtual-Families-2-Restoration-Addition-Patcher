...
try {
    for ($mask = $StartMask; $mask -lt 16; ++$mask) {
        ...
        $baseParts = @()
        if ($island) { $baseParts += "island_events" }
        if ($cheat) { $baseParts += "cheat_upgrades" }
        if ($holiday) { $baseParts += "holiday_ornaments" }
        $baseVariant = if ($baseParts.Count) { $baseParts -join "_" } else { "core" }

        $previous = Join-Path $outputs "VF2-Mobile-Furniture-With-Island-Events-B149-$baseVariant"
        $out = Join-Path $outputs "VF2-Mobile-Furniture-With-Island-Events-B150-$variant"
        if (-not (Test-Path -LiteralPath $previous -PathType Container)) {
            throw "Missing B149 base for ${variant}: $previous"
        }
        ...
        $patchLog = Join-Path $out "b150-patch.log"
        $buildLog = Join-Path $out "b150-build.log"
...
Write-Host "B150 16-state executable matrix completed successfully."
