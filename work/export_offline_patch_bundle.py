...
FINAL_PLAYTEST_DEFAULT_ON_SETTINGS = frozenset({
    "island_events",
...
NO_AI_ICON_TARGETS = (
    ...
    "cheat_clean_house.png",
    "cheat_fill_yard_weeds.png",
    "cheat_clean_garden.png",
    "cheat_max_sock_pile.png",
    "cheat_no_sock_pile.png",
    "cheat_marriage_email.png",
    "cheat_next_babies_male.png",
    "cheat_next_babies_female.png",
    "cheat_force_pregnancy.png",
    # The current Fix all house malfunctions row (item 0x12D) uses the
    # existing reset-achievements icon as its restore target.  The supplied
    # replacement is the distinct Resort Improvement art below.
    "cheat_reset_achievements.png",
    "cheat_trophy_gold2x.png",
    "cheat_next_pregnancy_singleton.png",
    "cheat_next_pregnancy_twins.png",
    "cheat_next_pregnancy_triplets.png",
)
NO_AI_ICON_REPLACEMENT_FILES = {
    # This normalized 90x90 file was generated by work/build_no_ai_icons.py
    # from the tracked source_art/Icon_Resort_Improvement.png input.
    "cheat_reset_achievements.png": "cheat_fix_malfunctions.png",
    "cheat_trophy_gold2x.png": "cheat_trophy_gold2x.png",
}
NO_AI_ICON_REPLACEMENT_PROVENANCE = {
    "cheat_reset_achievements.png": "patcher_assets/optional_patches/no_ai_icons/source_art/Icon_Resort_Improvement.png",
    "cheat_trophy_gold2x.png": "patcher_assets/optional_patches/no_ai_icons/source_art/trophy_gold2x.png",
}
try:
...
SETTINGS = [
    ...
    },
    {
        ...
        "description": "Enables the behavior-only executable overlay. B150 adds spontaneous Needs to sit down, Mending a button, Ironing clothes, Checking weight, and nursing-mother Teaching first words/infant care; enables the registered web, nap, sit-down, sink/grooming, snow, shower, meal, career, play, and other label routes while preserving their documented native age, object, weather, nursing, and gender gates; keeps Petting non-spontaneous; directly preserves the exact current action string across normal praise; and lets exact opposite-sex adult spouse pairs with six children use native Having Private Romantic Time with 0% pregnancy and no argument. All Behavior Patch native changes and variations are absent when this setting is disabled.",
        "default": True,
        "category": "main",
    },
    {
    ...
    },
    {
        ...
        "description": "Optional patch: enables ported actions for genuine mobile furniture where implemented. B156 makes good-weather loungers choose among relaxing, reading, studying, sitting, napping, and sleeping with exhaustion-sensitive rest odds, plus spontaneous supported variants. Exact guarded manual routes cover the Patio Umbrella and tables, Picnic Table, Birthday furniture, Christmas Trees, Dreidel, Menorah, Stockings, Holiday Candles, Santa's Cookie Plate, ten Holiday figurines, Red Bow, Santa Wall Decoration, and both garlands. Invisible/custom/VF3 furniture is excluded. The Picnic and Patio Tables draw their meal and drinks props: those two prop ids sit outside the engine's prop array, so their state is tracked separately and a wrapper on CDecal::RefreshProps draws them after the stock props. Whether they look right in play is not confirmed.",
        "default": True,
        "category": "optional",
    },
    {
    ...
    },
    {
        ...
        "label": "No AI Icons",
        "description": (
            "Requires Cheat Upgrades. Replaces only the late Special Upgrade icon PNGs with replacement images "
...
B150_CHANGELOG_LINES = (
    ...
    "- B158 Behavior Patches private time: An exact current-generation opposite-sex adult spouse pair with six children can use the native Having Private Romantic Time sequence. The native refusal/argument route is bypassed for that pair, and the TryToMakeBaby entry returns before ChanceOfPregnancy/Impregnate so pregnancy remains 0%; the rule is absent when Behavior Patches is disabled.",
    ...
    "- B150 reversible upgrades: Under Cheat Upgrades, rebuying Maid or Gardener at the active zero-price row clears the worker timer/active villager and fires that worker. Anti-Spam Software and Rockhound Certificate are independently repurchaseable in every patched executable, including saves where the effect flag is already cleared; buying an active row removes its state at zero price. Rebuying an owned native house renovation `0xE1-0xEA` returns that upgrade, reloads the base content map, reapplies the remaining native renovation records, and saves. Explicit cheat-overlay guards keep the other removal hooks inert when Cheat Upgrades is disabled.",
...
B180_CHANGELOG_LINES = (
    ...
    "- B180 stock-donor furniture: The Invisible Kiddie Pool, Invisible Full-Size Pool, Invisible Hammock, Invisible Yoga Equipment, Exercise Bike, Home Gym System and Ping-Pong Table borrow stock desktop furniture and are not routed through this dispatcher. The Exercise Bike, Home Gym System, Yoga Equipment and Ping-Pong Table now have villager actions of their OWN rather than relabelled donor actions: each carries its own behaviour id bound to its own handler, reusing the donor's animations and duration but not its identity. The Home Gym System had no action at all before -- it could be bought, placed and never used -- and now has one with ten workout variations. Owning an item never gates a behaviour; it adds an action and removes none, and the stock Treadmill and Pool Table keep their own behaviour and labels unchanged. Note this is what a villager CHOOSES to do on their own. Whether a villager DROPPED on one of these reacts is a separate question and is NOT claimed: that path is the game's own hotspot dispatch, which cannot tell one added item from another.",
    ...
    "- Picnic and patio table props: the meal and drinks are drawn on the tables. CEnvironment::SetProp accepts prop ids 0x00-0x54 only, and these two are 0x55 and 0x56, so passing either would write out of bounds; their state is tracked outside the engine array instead and a wrapper on CDecal::RefreshProps draws them after the stock props. Whether they look right in play is not confirmed.",
)
...
def setting_for_native_source(path_parts: list[str]) -> str:
    ...
    if "settings_menu/evict" in path_text:
        # Settings Evict is compiled into the core executable; it has no
        # independent exporter toggle or overlay.
...
def setting_for_asset(rel_path: Path) -> str:
    ...
    if text.startswith("OptionalVisualMods/Invisible Furniture Backups/"):
        return "invisible_furniture_transparent_graphics"
    if text.startswith("OptionalVisualMods/No AI Icons/"):
        return "no_ai_icons"
...
def no_ai_icon_asset_patches(
    bundle_dir: Path,
    build_dir: Path,
    base_payload: Path,
) -> list[dict[str, Any]]:
    """Add the gated No AI Icons replacements with self-contained restores.

    The generated Cheat Upgrades icon records remain the normal active layer.
    Each replacement record uses a distinct requirement signature so the
    offline patcher can layer it over that normal icon when enabled.  A
    ``restore_requires`` gate keeps the restore path valid only while Cheat
    Upgrades is still enabled; if Cheat Upgrades is disabled at the same time,
    its normal remove records win and the replacement record is ignored.
    """

    payload_root = bundle_dir / "payload"
    sources = [
        NO_AI_ICON_SOURCE_DIR / NO_AI_ICON_REPLACEMENT_FILES.get(filename, filename)
        for filename in NO_AI_ICON_TARGETS
    ]
    missing = [path for path in sources if not path.is_file()]
    if missing:
        raise ValueError(
            "No AI Icons source set is incomplete; missing: "
            + ", ".join(str(path.relative_to(NO_AI_ICON_SOURCE_DIR)) for path in missing)
        )
    for source in sources:
        data = source.read_bytes()
        if len(data) < 24 or data[:8] != b"\x89PNG\r\n\x1a\n":
            raise ValueError(f"No AI Icons source is not a PNG: {source}")
        width, height = struct.unpack(">II", data[16:24])
        if (width, height) != (90, 90):
            raise ValueError(
                f"No AI Icons source must be 90x90: {source.name} is {width}x{height}"
            )

    records: list[dict[str, Any]] = []
    custom_root = payload_root / "OptionalVisualMods" / "No AI Icons"
    custom_root.mkdir(parents=True, exist_ok=True)
    for filename in NO_AI_ICON_TARGETS:
        source = NO_AI_ICON_SOURCE_DIR / NO_AI_ICON_REPLACEMENT_FILES.get(filename, filename)
        custom_target = custom_root / filename
        custom_target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, custom_target)

        restore_target = payload_root / "Images" / filename
        if not restore_target.is_file():
            candidates = (
                build_dir / "Images" / filename,
                base_payload / "Images" / filename,
            )
            restore_source = next((candidate for candidate in candidates if candidate.is_file()), None)
            if restore_source is None:
                raise ValueError(
                    "No AI Icons requires the current Cheat Upgrades icon as a restore source: "
                    f"Images/{filename}"
                )
        ...
        records.append(
            {
                ...
                "source_path": relative_posix(custom_target.relative_to(bundle_dir)),
                "source_sha256": sha256_file(custom_target),
                "source_size": custom_target.stat().st_size,
                "restore_source_path": relative_posix(restore_target.relative_to(bundle_dir)),
                "restore_source_sha256": sha256_file(restore_target),
                "restore_source_size": restore_target.stat().st_size,
                "allow_missing_target": True,
                "overwrite_existing": True,
                "requires": ["core_executable", "cheat_upgrades", "no_ai_icons"],
                "restore_requires": ["core_executable", "cheat_upgrades"],
                "note": (
                    "No AI Icons replacement for a late Special Upgrade; disabling this setting "
                    "restores the current Cheat Upgrades icon."
                ),
            }
        )
    return records
...
def apply_final_playtest_defaults(
    ...
    """Enable only the certified working feature set for a final playtest.
    ...
    if missing:
        raise ValueError(
            "Final playtest profile is missing required feature records: "
            + ", ".join(sorted(missing))
        )
    for row in settings:
...
def write_transparency_log(bundle_dir: Path, manifest: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        "- Optional Patches (black): Holiday Ornaments, Island Events, Allow Older Pregnancies, Older Villager Mortality Curve, mobile furniture behaviors, No AI Icons, optional visual swaps, Invisible Furniture graphics modes, custom maps, LDW Posters/Paintings, and Colorful Couches.",
        "- Settings Evict is compiled into the core executable patch and is not an independent optional setting.",
        "",
        ...
        "- Cheat rows, price multipliers/reset, Trigger/Fix malfunction actions, Router offline/online changes, and reversible Maid/Gardener handling require cheat_upgrades. Anti-Spam Software and Rockhound Certificate retain an independent repurchase/removal route in every patched executable.",
...
def build_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.include_exe_replacement and cheat_upgrades_exe is not None:
        icon_defaults_available = all(
            (
                (bundle_dir / "payload" / "Images" / filename).is_file()
                or (cheat_upgrades_exe.parent / "Images" / filename).is_file()
                or (base_payload / "Images" / filename).is_file()
            )
            for filename in NO_AI_ICON_TARGETS
        )
        if icon_defaults_available:
            asset_patches.extend(
                no_ai_icon_asset_patches(
                    bundle_dir,
                    cheat_upgrades_exe.parent,
                    base_payload,
                )
            )
    ...
    if final_profile is not None:
        manifest["final_playtest_profile"] = final_profile
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Mark the certified working feature settings default-on in this exported bundle only "
            "(Island Events, Holiday Ornaments, Behavior Patches, Mobile Renovations, Mobile Sounds, "
            "Mobile Furniture Behaviors, and Cheat Upgrades). No AI Icons and unrelated visual options remain off."
        ),
    )
