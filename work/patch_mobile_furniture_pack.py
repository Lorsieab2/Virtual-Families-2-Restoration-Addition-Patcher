...
# Holiday Ornaments are a complete optional collection overlay.  The offline
# patcher selects a matching executable built with this gate enabled.
...
OLDER_MORTALITY_BASE_INTENSITY_PER_YEAR = 0.00365
OLDER_MORTALITY_LATE_INTENSITY_PER_YEAR = 0.06
OLDER_MORTALITY_LATE_ACCELERATION_EFFECTIVE_AGE = 110


def older_mortality_hazard_millionths(effective_age):
    """Return the B155.5 annual old-age threshold in the 0..999999 domain."""
    years_past_threshold = max(0, effective_age - OLDER_MORTALITY_TABLE_FIRST_AGE)
    late_years = max(
        0,
        effective_age - OLDER_MORTALITY_LATE_ACCELERATION_EFFECTIVE_AGE,
    )
    intensity = (
        OLDER_MORTALITY_BASE_INTENSITY_PER_YEAR * years_past_threshold
        + OLDER_MORTALITY_LATE_INTENSITY_PER_YEAR * late_years
    )
    hazard = -math.expm1(-intensity)
    # Round half up to the nearest millionth, then retain one non-death outcome.
    threshold = int(math.floor(hazard * OLDER_MORTALITY_RANDOM_LIMIT + 0.5))
    return min(OLDER_MORTALITY_HAZARD_CAP_MILLIONTHS, threshold)


# The first quantized cap occurs at effective age 318. Store every non-cap
# threshold explicitly in the native helper and use the cap thereafter.
OLDER_MORTALITY_TABLE_LAST_AGE = 317
OLDER_MORTALITY_HAZARDS_MILLIONTHS = tuple(
    older_mortality_hazard_millionths(age)
    for age in range(
        OLDER_MORTALITY_TABLE_FIRST_AGE,
        OLDER_MORTALITY_TABLE_LAST_AGE + 1,
    )
)
if len(OLDER_MORTALITY_HAZARDS_MILLIONTHS) != (
...
def older_mortality_survival_probability_through_age(age, active_food_groups=0):
    ...
    for displayed_age in range(OLDER_MORTALITY_TABLE_FIRST_AGE, age + 1):
        hazard = older_mortality_hazard_millionths(displayed_age - groups)
        probability *= 1.0 - hazard / OLDER_MORTALITY_RANDOM_LIMIT
    return probability
...
OFFICIAL_B93_RELEASE_SHA256 = "63ad60cfb963008bed7cc6706f05146ed7ed6a8f40aa785204c9ccefa36dbf55"
OFFICIAL_B93_RELEASE_SIZE = 353946169
...
HOLIDAY_DETAIL_BODY_CELL_SIZE = 164
HOLIDAY_DETAIL_BODY_COLUMNS = {"male": 0, "female": 1}
HOLIDAY_DETAIL_BODY_IMAGE_COUNT = len(OUTFIT_STORE_GENDERS) * len(HOLIDAY_BODY_VALUES)
HOLIDAY_DETAIL_BODY_SOURCE_FILES = {
    ("female", 50): "FemaleBodies_051_0058.png",
    ("female", 51): "FemaleBodies_052_0057.png",
    ("female", 52): "FemaleBodies_053_0058.png",
    ("female", 53): "FemaleBodies_054_0057.png",
    ("male", 50): "MaleBodies_0051_0058.png",
    ("male", 51): "MaleBodies_0052_0058.png",
    ("male", 52): "MaleBodies_0053_0058.png",
    ("male", 53): "MaleBodies_0054_0057.png",
}
...
HOLIDAY_ORNAMENT_MASTER_COLLECTOR_ID = 0x4D
...
HOLIDAY_ORNAMENT_PREEXTRACTED_ART_DIR = HOLIDAY_ORNAMENT_SUPPLIED_ART_DIR / "collection"
...
# Exact mobile 1.7.16 page-five coordinates from
# CCollectionScene::sm_sCollectable.  The fallback page and icons are scaled
# from 800x600 to 1024x768, so descriptor coordinates use the same 1.28 scale.
# The engine interprets Y from the bottom; no top-origin conversion belongs in
# this descriptor table.
HOLIDAY_ORNAMENT_MOBILE_SLOT_POSITIONS = [
    (0x092, 0x087),
    (0x11D, 0x088),
    (0x1A5, 0x087),
    (0x226, 0x082),
...
HOLIDAY_ORNAMENT_COLLECTION_SLOT_POSITIONS = [
    (
        round(x * HOLIDAY_ORNAMENT_IMAGE_SCALE),
        round(y * HOLIDAY_ORNAMENT_IMAGE_SCALE),
    )
    for x, y in HOLIDAY_ORNAMENT_MOBILE_SLOT_POSITIONS
]
...
def build_native_array_contract():
    ...
    return {
        ...
        "holiday_ornaments": {
            ...
            "status": (
                "enabled mobile-parity six-page collection"
                if ENABLE_HOLIDAY_ORNAMENTS
                else "optional patch not selected; stock PC Collections UI remains 60 collectibles"
            ),
            "requirements": [
                ...
                "use hash-verified workspace Holiday page art and collectables_small.png copied into the modified build folder",
            ],
        },
    }
...
def is_added_mobile_holiday_or_vf3_item(data):
    pack = str(data.get("custom_pack", ""))
    return data.get("mobile_row") is not None or pack.startswith("VF3 ")


def evenly_spaced_generation_values(count):
    generations = list(ADDED_FURNITURE_LOCK_GENERATIONS)
    if count <= 0:
        return []
    if count >= len(generations):
        return generations
    if count == 1:
        return [generations[0]]
    indexes = [
        round(i * (len(generations) - 1) / (count - 1))
    ...
    for index in indexes:
        if index in used:
            for candidate in range(len(generations)):
                if candidate not in used:
                    index = candidate
                    break
        selected.append(generations[index])
        used.add(index)
    return selected


def assign_added_generation_locks(paths):
    if not paths:
        return {}, []
    rng = random.Random(ADDED_FURNITURE_LOCK_SEED)
    shuffled_paths = list(paths)
    rng.shuffle(shuffled_paths)

    item_count = len(shuffled_paths)
    min_groups = (item_count + ADDED_FURNITURE_LOCK_MAX_GROUP_SIZE - 1) // ADDED_FURNITURE_LOCK_MAX_GROUP_SIZE
    max_groups = item_count // ADDED_FURNITURE_LOCK_MIN_GROUP_SIZE
    if max_groups <= 0:
        group_count = item_count
    else:
        group_count = min(len(ADDED_FURNITURE_LOCK_GENERATIONS), max_groups)
        group_count = max(group_count, min_groups)
    group_count = max(1, min(group_count, len(shuffled_paths), len(ADDED_FURNITURE_LOCK_GENERATIONS)))

    generations = evenly_spaced_generation_values(group_count)
    rng.shuffle(generations)

    base_size = item_count // group_count
    extra = item_count % group_count
    group_sizes = [base_size] * group_count
    for index in rng.sample(range(group_count), extra):
        group_sizes[index] += 1

    assignments = {}
    groups = []
    cursor = 0
    for group_index, (generation, group_size) in enumerate(zip(generations, group_sizes), start=1):
        group_paths = shuffled_paths[cursor : cursor + group_size]
        cursor += group_size
        for path in group_paths:
            assignments[path] = generation
        groups.append({
            "group": group_index,
            "generation": generation,
            "item_count": len(group_paths),
            "paths": group_paths,
        })
...
def apply_generation_lock_distribution():
    ...
    for idx, (_name, _donor, _list_name, path) in enumerate(ITEMS):
        ...
        original_generation = int(data.get("lock_generation", 0))
        data["original_lock_generation"] = original_generation
        pack = data.get("custom_pack", "")
        if pack.startswith("Invisible "):
            generation = 0
            reason = "invisible furniture remains placement/debug unlocked"
        elif original_generation <= 0 and is_added_mobile_holiday_or_vf3_item(data):
            eligible_paths.append(path)
            generation = 0
            reason = "pending deterministic added-furniture lock assignment"
        else:
            generation = original_generation
            reason = "retained original/additive lock"
        data["lock_generation"] = generation
        data["assigned_lock_generation"] = generation
        data["generation_lock_assignment_reason"] = reason

    assignments, groups = assign_added_generation_locks(eligible_paths)
    for path, generation in assignments.items():
        data = MOBILE_DATA_BY_PATH[path]
        data["lock_generation"] = generation
        data["assigned_lock_generation"] = generation
        data["generation_lock_assignment_reason"] = (
            "deterministically shuffled mobile/Holiday/VF3 unlocked item group"
        )

    global GENERATION_LOCK_ASSIGNMENT_SUMMARY
    GENERATION_LOCK_ASSIGNMENT_SUMMARY = {
        "seed": ADDED_FURNITURE_LOCK_SEED,
        "generation_range": [min(ADDED_FURNITURE_LOCK_GENERATIONS), max(ADDED_FURNITURE_LOCK_GENERATIONS)],
        "group_size_target": [
            ADDED_FURNITURE_LOCK_MIN_GROUP_SIZE,
            ADDED_FURNITURE_LOCK_MAX_GROUP_SIZE,
        ],
        "eligible_count": len(eligible_paths),
        "assigned_count": len(assignments),
        "groups": groups,
        "scope": "added mobile/Holiday/VF3 furniture records whose original lock_generation was 0",
        "base_game_policy": "base-game furniture records are not part of ITEMS and are not modified",
    }
...
def villager_sprite_copy_source_dirs():
    ...
    if env_source:
        roots.append(Path(env_source))
    roots.extend(VILLAGER_SPRITE_COPY_SOURCE_DIRS)
...
def villager_detail_body_image_base():
    return villager_body_image_base() + HOLIDAY_BODY_IMAGE_COUNT


def villager_detail_body_image_index(gender, body_value):
    gender_index = OUTFIT_STORE_GENDERS.index(gender)
    body_index = HOLIDAY_BODY_VALUES.index(body_value)
    return gender_index * len(HOLIDAY_BODY_VALUES) + body_index
...
def holiday_body_descriptor_count():
    return HOLIDAY_BODY_IMAGE_COUNT + HOLIDAY_DETAIL_BODY_IMAGE_COUNT
...
def sync_original_villager_sprite_sheets(manifest):
    ...
    for filename in VILLAGER_SPRITE_SHEET_FILES:
        ...
        source = next(
            (
                root / filename
                for root in villager_sprite_copy_source_dirs()
    ...
    manifest["villager_sprite_sheet_copy"] = {
        "status": "available in build-local Images folder" if (copied or preserved) and not missing and not issues else "partial_or_failed",
        "source_dirs": [str(root) for root in villager_sprite_copy_source_dirs()],
        "runtime_images_dir": "Images",
        "runtime_note": "The game uses Images/*.png in the modified build folder; if the seeded runtime already includes these sheets they are preserved in place.",
        "copied": copied,
        "preserved": preserved,
        "missing": missing,
        "issues": issues,
...
def bundled_vf3_tv_animation_sheet(label):
    runtime_name = VF3_TV_RUNTIME_ANIMATION_NAMES.get(label)
    if not runtime_name:
        return None
    return VF3_TV_ANIMATION_ASSET_DIR / runtime_name


def copy_vf3_tv_animation_sheet_cells(source_sheet, frame_dir):
    copied_frames = []
    present = 0
            ...
            if not supplied_sheet or not supplied_sheet.exists():
                for index in range(1, 19):
                    ...
                    with Image.open(source).convert("RGBA") as frame:
                        ...
                        present += 1
            bundled_sheet = bundled_vf3_tv_animation_sheet(label)
            if present < 18 and bundled_sheet and bundled_sheet.exists():
                previous_present = present
                with Image.open(bundled_sheet).convert("RGBA") as source_sheet:
                    source_bbox = source_sheet.getchannel("A").getbbox()
                    if source_bbox:
                        sheet = source_sheet.copy()
                        cell_w = source_sheet.width // 6
                        cell_h = source_sheet.height // 3
                        present, bundled_frames = copy_vf3_tv_animation_sheet_cells(source_sheet, frame_dir)
                        copied.append({
                            "source_sheet": str(bundled_sheet),
                            "status": "used_workspace_bundled_runtime_strip",
                            "frames": present,
                            "grid": [6, 3],
                            "cell": [cell_w, cell_h],
                            "size": list(source_sheet.size),
                            "replaced_generated_frame_count": previous_present,
                            "frame_exports": bundled_frames,
                        })
                    else:
                        missing.append(f"{bundled_sheet}: bundled runtime strip is blank")
...
def _alpha_composite_unscaled_clipped(output, cropped, paste_x, paste_y):
    dst_x = max(paste_x, 0)
    dst_y = max(paste_y, 0)
    src_x = max(-paste_x, 0)
    src_y = max(-paste_y, 0)
    width = min(cropped.width - src_x, output.width - dst_x)
    height = min(cropped.height - src_y, output.height - dst_y)
    if width <= 0 or height <= 0:
        return
    output.alpha_composite(
        cropped.crop((src_x, src_y, src_x + width, src_y + height)),
        (dst_x, dst_y),
    )
...
def _normalize_holiday_body_frame(source_image, target_template):
    ...
    if source_image.size == (HOLIDAY_BODY_CELL_SIZE, HOLIDAY_BODY_CELL_SIZE):
        return source_image.copy()
    source_width, source_height = cropped.size
    target_center_x = (target_bbox[0] + target_bbox[2]) // 2
    target_bottom_y = target_bbox[3]
    paste_x = target_center_x - source_width // 2
    paste_y = target_bottom_y - source_height
    _alpha_composite_unscaled_clipped(output, cropped, paste_x, paste_y)
    return output
...
def _normalize_holiday_detail_body_frame(source_image, target_template):
    from PIL import Image

    source_bbox = _alpha_bbox(source_image)
    target_bbox = _alpha_bbox(target_template)
    if not source_bbox or not target_bbox:
        return Image.new("RGBA", source_image.size, (0, 0, 0, 0))
    if source_image.size == (HOLIDAY_DETAIL_BODY_CELL_SIZE, HOLIDAY_DETAIL_BODY_CELL_SIZE):
        return source_image.copy()
    cropped = source_image.crop(source_bbox)
    output = Image.new("RGBA", (HOLIDAY_DETAIL_BODY_CELL_SIZE, HOLIDAY_DETAIL_BODY_CELL_SIZE), (0, 0, 0, 0))
    source_width, source_height = cropped.size
    target_center_x = (target_bbox[0] + target_bbox[2]) // 2
    target_bottom_y = target_bbox[3]
    paste_x = target_center_x - source_width // 2
...
def sync_holiday_detail_body_frames(manifest):
    """Generate one-cell Details-screen body frames for Holiday body rows.

    The profile/details screen uses highrez_bodies_final2.png, a 2x50 sheet
    with gender columns and body rows. Keep that stock sheet unchanged and
    register separate one-cell images for Holiday rows 50-53.
    """
    from PIL import Image

    output_root = OUT / "Images" / "VillagerDetailBodies"
    frames = []
    missing = []
    issues = []
    source_sheet, skips = _find_source_image(
        "highrez_bodies_final2.png",
        min_width=HOLIDAY_DETAIL_BODY_CELL_SIZE * len(HOLIDAY_DETAIL_BODY_COLUMNS),
        min_height=HOLIDAY_DETAIL_BODY_CELL_SIZE * HOLIDAY_BODY_BASE_ROWS,
    )
    if not source_sheet:
        missing.append("highrez_bodies_final2.png")
        missing.extend(skips)
        manifest["holiday_detail_body_frames"] = {
            "status": "failed_or_missing_template",
            "root": str(output_root),
            "body_values": list(HOLIDAY_BODY_VALUES),
            "source_dir": str(HOLIDAY_DETAIL_BODY_SOURCE_DIR),
            "frames": frames,
            "missing": missing,
            "issues": issues,
        }
    ...
    try:
        ...
        with Image.open(source_sheet).convert("RGBA") as sheet:
            for gender in OUTFIT_STORE_GENDERS:
                column = HOLIDAY_DETAIL_BODY_COLUMNS[gender]
                template_box = (
                    column * HOLIDAY_DETAIL_BODY_CELL_SIZE,
                    (HOLIDAY_BODY_BASE_ROWS - 1) * HOLIDAY_DETAIL_BODY_CELL_SIZE,
                    (column + 1) * HOLIDAY_DETAIL_BODY_CELL_SIZE,
                    HOLIDAY_BODY_BASE_ROWS * HOLIDAY_DETAIL_BODY_CELL_SIZE,
                )
                template = sheet.crop(template_box)
                gender_title = gender.title()
                for body_value in HOLIDAY_BODY_VALUES:
                    source_name = HOLIDAY_DETAIL_BODY_SOURCE_FILES[(gender, body_value)]
                    source = HOLIDAY_DETAIL_BODY_SOURCE_DIR / gender_title / source_name
                    source_kind = "supplied_detail_frame"
                    if source.is_file():
                        with Image.open(source).convert("RGBA") as raw_frame:
                            normalized = _normalize_holiday_detail_body_frame(raw_frame, template)
                    else:
                        missing.append(str(source))
                        normalized = template.copy()
                        source_kind = "stock_row49_fallback"
                    ...
                    frames.append({
                        ...
                        "template": str(source_sheet),
                    })
    except Exception as exc:
        issues.append({"reason": str(exc)})

    manifest["holiday_detail_body_frames"] = {
        "status": "generated" if len(frames) == HOLIDAY_DETAIL_BODY_IMAGE_COUNT else "partial_or_failed",
        "root": str(output_root),
        "body_values": list(HOLIDAY_BODY_VALUES),
        "source_dir": str(HOLIDAY_DETAIL_BODY_SOURCE_DIR),
        ...
        "missing": missing,
        "issues": issues,
        "runtime_note": "Details screen highrez body rows 50-53 draw from these generated one-cell images; missing files fall back to stock row 49.",
    }
...
def sync_outfit_store_icon_art(manifest):
    ...
    try:
        for entry in outfit_store_entries():
            ...
            entries.append({
                ...
                "gender": gender,
                "body_value": body_value,
                "image_id": hex(outfit_icon_image_id(gender, body_value, holiday_body_descriptor_count() if ENABLE_HOLIDAY_BODY_TYPES else 0)),
    ...
    manifest["outfit_store_icons"] = {
        ...
        "root": str(output_root),
        "image_base": hex(outfit_icon_image_base(holiday_body_descriptor_count() if ENABLE_HOLIDAY_BODY_TYPES else 0)),
        "source_rule": {
...
def validate_holiday_ornament_native_contract(manifest):
    ...
    load_data, _load_sym, _load_sec = function_bytes(
        collectable_obj, "?LoadState@CCollectableItem@@QAE?B_NABUSSaveState@1@@Z"
    )
    if (
        b"\xBE" + struct.pack("<I", stock_collection_state_entries)
    ) not in load_data or (
        b"\x8D\x8F" + struct.pack("<I", stock_collection_state_start)
    ) not in load_data or (
        b"\x8D\x93\x48\x03\x00\x00"
    ) not in load_data:
        errors.append(
            "CCollectableItem::LoadState no longer restores the full "
            "0xAF-entry collection-state table"
        )
    ...
    for function_name, purpose in (
        ...
        (
            ...
            "exact carrying-value pickup search",
        ),
        (
            "?WasItemSpawned@CCollectableItem@@QBE?B_NW4ECarrying@@@Z",
            "exact carrying-value duplicate check",
        ),
    ):
        patched_data, _patched_sym, _patched_sec = function_bytes(
            collectable_obj,
            function_name,
        )
        stock_data, _stock_sym, _stock_sec = function_bytes(
            stock_collectable_obj,
            function_name,
        )
        if patched_data != stock_data:
            errors.append(
                f"{function_name} no longer retains stock/mobile-parity "
                f"{purpose}"
            )

    spawn_reset_data, spawn_reset_sym, spawn_reset_sec = function_bytes(
        collectable_obj,
        "?Reset@CCollectableItem@@QAEXXZ",
    )
    stock_spawn_reset_sym = stock_collectable_obj.symbol(
        "?Reset@CCollectableItem@@QAEXXZ"
    )
    stock_spawn_reset_sec = stock_collectable_obj.section(
        stock_spawn_reset_sym.section
    )
    add_spawn_name = (
        "?AddSpawnArea@CCollectableItem@@"
        "QAEXUldwRect@@W4ECarrying@@@Z"
    )
    add_spawn_symbol = collectable_obj.symbol(add_spawn_name)
    stock_add_spawn_symbol = stock_collectable_obj.symbol(add_spawn_name)
    spawn_reset_relocs = [
        struct.unpack_from(
            "<IIH",
            collectable_obj.buf,
            spawn_reset_sec.reloc_ptr + index * 10,
        )
        for index in range(spawn_reset_sec.nreloc)
    ]
    stock_spawn_reset_relocs = [
        struct.unpack_from(
            "<IIH",
            stock_collectable_obj.buf,
            stock_spawn_reset_sec.reloc_ptr + index * 10,
        )
        for index in range(stock_spawn_reset_sec.nreloc)
    ]
    stock_spawn_call_count = sum(
        symbol_index == stock_add_spawn_symbol.index
        for _vaddr, symbol_index, _rtype in stock_spawn_reset_relocs
    )
    patched_spawn_call_count = sum(
        symbol_index == add_spawn_symbol.index
        for _vaddr, symbol_index, _rtype in spawn_reset_relocs
    )
    expected_spawn_call_count = (
        stock_spawn_call_count + len(HOLIDAY_ORNAMENT_SPAWN_RECTS)
    )
    if patched_spawn_call_count != expected_spawn_call_count:
        errors.append(
            "CCollectableItem::Reset expected "
            f"{expected_spawn_call_count} total spawn registrations, got "
            f"{patched_spawn_call_count}"
        )
    spawn_insert = spawn_reset_sym.value + 0x1ED
    relocation_targets = {
        vaddr: collectable_obj.symbol_by_index[symbol_index].name
        for vaddr, symbol_index, _rtype in spawn_reset_relocs
    }
    for index, (rect_symbol, _rect) in enumerate(
        HOLIDAY_ORNAMENT_SPAWN_RECTS
    ):
        record_off = spawn_insert + index * 27
        expected_record = (
            b"\x0F\x28\x05\x00\x00\x00\x00"
            b"\x8B\xCB\x68"
            + struct.pack("<I", HOLIDAY_ORNAMENT_COLLECTABLE_START)
            + b"\x83\xEC\x10\x8B\xC4\x0F\x11\x00"
            b"\xE8\x00\x00\x00\x00"
        )
        if (
            spawn_reset_data[
                record_off - spawn_reset_sym.value :
                record_off - spawn_reset_sym.value + len(expected_record)
            ]
            != expected_record
        ):
            errors.append(
                f"CCollectableItem::Reset Holiday spawn record {index} "
                "does not match the fixed 27-byte AddSpawnArea sequence"
            )
        if relocation_targets.get(record_off + 3) != rect_symbol:
            errors.append(
                f"CCollectableItem::Reset Holiday spawn record {index} "
                "uses the wrong mobile rectangle"
            )
        if relocation_targets.get(record_off + 23) != add_spawn_name:
            errors.append(
                f"CCollectableItem::Reset Holiday spawn record {index} "
                "does not call AddSpawnArea"
            )
    ...
    sm_collectable = collection_scene_obj.symbol(
        "?sm_sCollectable@CCollectionScene@@0PAUSCollectable@1@A"
    )
    sm_collectable_sec = collection_scene_obj.section(sm_collectable.section)
    ornament_slots_raw = (
        sm_collectable_sec.raw_ptr
        + sm_collectable.value
        + 5 * HOLIDAY_ORNAMENT_COLLECTION_ITEM_COUNT * 12
    )
    ornament_slots = [
        struct.unpack_from(
            "<III",
            collection_scene_obj.buf,
            ornament_slots_raw + index * 12,
        )
        for index in range(HOLIDAY_ORNAMENT_COLLECTION_ITEM_COUNT)
    ]
    expected_ornament_slots = [
        (
            holiday_ornament_collection_item_image_id(
                index,
                holiday_desc_count,
            ),
            *HOLIDAY_ORNAMENT_COLLECTION_SLOT_POSITIONS[index],
        )
        for index in range(HOLIDAY_ORNAMENT_COLLECTION_ITEM_COUNT)
    ]
    if ornament_slots != expected_ornament_slots:
        errors.append(
            "CCollectionScene::sm_sCollectable does not contain the "
            "12 mobile-order Holiday Ornament slots"
        )
    ...
    draw_cave = None
    if (
        draw_data[0x17D] != 0xE9
        or draw_data[0x182 : 0x184] != b"\x90\x90"
    ):
        errors.append(
            "CCollectionScene::DrawScene page-count lookup is not a "
            "fixed-size seven-byte detour"
        )
    else:
        draw_cave = (
            0x17D
            + 5
            + struct.unpack_from("<i", draw_data, 0x17E)[0]
        )
        expected_draw_cave = (
        ...
        if (
            draw_data[
                draw_cave : draw_cave + len(expected_draw_cave)
            ]
            != expected_draw_cave
        ):
            errors.append(
                "CCollectionScene::DrawScene code cave does not call the "
                "Holiday page-count helper and push its result"
            )
        else:
            return_target = (
                draw_cave
            ...
            if return_target != 0x184:
                errors.append(
                    "CCollectionScene::DrawScene page-count cave does not "
                    "resume at the first untouched instruction"
                )
    ...
    if helper_symbol is None:
        ...
        if draw_cave is None or not any(
            va == draw_sym.value + draw_cave + 4
            and idx == helper_symbol.index
            and typ == IMAGE_REL_I386_REL32
            for va, idx, typ in relocs
        ):
    ...
    collectable_item_manifest = manifest.get(
        "CollectableItemHolidayOrnaments",
        {},
    )
    if (
        collectable_item_manifest.get("spawn_area_count")
        != len(HOLIDAY_ORNAMENT_SPAWN_RECTS)
    ):
        errors.append(
            "Holiday Ornament manifest does not record the three mobile "
            "spawn regions"
        )
    if collectable_item_manifest.get("mobile_spawn_rects") != [
        [hex(value) for value in rect]
        for _symbol, rect in HOLIDAY_ORNAMENT_SPAWN_RECTS
    ]:
        errors.append(
            "Holiday Ornament manifest spawn rectangles do not match mobile "
            "1.7.16"
        )
    ...
    if not the_collector.get("keep_choice_branch", {}).get("unchanged"):
        errors.append("The Collector keep-choice stock branch is not recorded as unchanged")
    if [
        row.get("operand_offset")
        for row in the_collector.get("offer_count_relocations", [])
    ] != ["0x88", "0xfb", "0x171"]:
        errors.append(
            "The Collector manifest does not record the three relocation-only "
            "Holiday rarity counts"
        )

    achievement_obj = CoffObject(PATCHED / "Achievement.obj")
    achievements_complete_data, _complete_count_sym, _complete_count_sec = (
        function_bytes(
            achievement_obj,
    ...
    achievement_list_sec = achievement_obj.section(achievement_list.section)
    for achievement_id, expected_target, label in (
        (
            HOLIDAY_ORNAMENT_MASTER_COLLECTOR_ID,
            HOLIDAY_ORNAMENT_MASTER_COLLECTOR_TARGET,
            "Master Collector",
        ),
        (
            HOLIDAY_ORNAMENT_GOAL_COLLECTOR_ID,
            HOLIDAY_ORNAMENT_GOAL_COLLECTOR_TARGET,
            "Goal Collector",
        ),
        (
    ...
    canfire_data, canfire_sym, canfire_sec = function_bytes(
        island_obj,
        "?CanFire@CEventTheCollector@@UAE_NXZ",
    )
    stock_island_obj = CoffObject(SRC_OBJS / "IslandEvents.obj")
    stock_canfire_data, _stock_canfire_sym, _stock_canfire_sec = function_bytes(
        stock_island_obj,
        "?CanFire@CEventTheCollector@@UAE_NXZ",
    )
    if canfire_data != stock_canfire_data:
        errors.append(
            "CEventTheCollector::CanFire bytes changed even though B151 "
            "uses relocation-only Holiday offer hooks"
        )
    collector_count_helper_name = (
        "?CollectionCountWithHolidayOrnaments@CCollectableItem@@"
        "QBE?BHW4ECarrying@@_N11@Z"
    )
    collector_count_helper = island_obj.symbol_by_name.get(
        collector_count_helper_name
    )
    stock_count_symbol = island_obj.symbol(
    ...
    canfire_relocs = {
        ...
        for vaddr, symbol_index, rtype in (
            struct.unpack_from(
                "<IIH",
                island_obj.buf,
                canfire_sec.reloc_ptr + index * 10,
            )
            for index in range(canfire_sec.nreloc)
        )
    }
    if collector_count_helper is None:
        errors.append(
            "IslandEvents.obj has no Holiday aggregate count helper import"
        )
    else:
        for operand_off in (0x088, 0x0FB, 0x171):
            if canfire_relocs.get(
                canfire_sym.value + operand_off
            ) != (collector_count_helper.index, IMAGE_REL_I386_REL32):
                errors.append(
                    "CEventTheCollector::CanFire rarity call "
                    f"{operand_off:#x} does not include Holiday Ornaments"
                )
        if canfire_relocs.get(
            canfire_sym.value + 0x1AC
        ) != (stock_count_symbol.index, IMAGE_REL_I386_REL32):
            errors.append(
                "CEventTheCollector::CanFire final eligibility no longer "
                "matches mobile stock-family behavior"
            )

    impact_data, impact_sym, impact_sec = function_bytes(
        island_obj,
        "?ImpactGame@CEventTheCollector@@UAEXH@Z",
    )
    if impact_data[0x07 : 0x09] != b"\x75\x66":
        errors.append(
            "CEventTheCollector::ImpactGame keep-choice branch is not the "
    ...
    reset_helper = island_obj.symbol_by_name.get(reset_helper_name)
    impact_relocs = {
        vaddr: (symbol_index, rtype)
        for vaddr, symbol_index, rtype in (
            struct.unpack_from(
    ...
    if reset_helper is None or impact_relocs.get(
        impact_sym.value + 0x6B
    ) != (reset_helper.index, IMAGE_REL_I386_REL32):
        errors.append(
            "CEventTheCollector::ImpactGame sell tail does not route through "
            "the Holiday progress-reset helper"
        )
    if helper_source.is_file():
        if (
            "void CAchievement::ResetHolidayOrnamentCollectorProgress("
            not in helper_text
            or "ResetSingleAchievementProgress((EAchievement)0x5F);"
            not in helper_text
        ):
            errors.append(
                "Holiday Collector reset helper source does not reset "
                "Ornamentologist progress before the stock tail"
            )
    ...
    manifest["holiday_ornament_native_contract"] = {
        ...
        "collection_state": {
            ...
            "load_state_covers_holiday_range": True,
            "reset_collection_covers_holiday_range": True,
        },
        "spawning": {
            "stock_spawn_area_count": stock_spawn_call_count,
            "holiday_spawn_area_count": len(
                HOLIDAY_ORNAMENT_SPAWN_RECTS
            ),
            "total_spawn_area_count": expected_spawn_call_count,
            "mobile_spawn_rects": [
                [hex(value) for value in rect]
                for _symbol, rect in HOLIDAY_ORNAMENT_SPAWN_RECTS
            ],
        ...
        "collection_scene": {
            ...
            "slot_entries": [
                ...
                for image_id, x, y in ornament_slots
            ],
            "mobile_slot_positions": [
                [x, y]
                for x, y in HOLIDAY_ORNAMENT_MOBILE_SLOT_POSITIONS
            ],
        ...
        "pickup_dispatch": {
            ...
            "find": "stock exact ECarrying equality retained",
            "was_item_spawned": "stock exact ECarrying equality retained",
            "drop": (
                "first copies increment 0x5F; duplicates retain the stock "
                "coin-only route; the twelfth unique copy completes 0x4D"
            ),
        },
        "sell_all": {
            "event": "CEventTheCollector",
            "achievement_reset": hex(HOLIDAY_ORNAMENT_ACHIEVEMENT_ID),
            "offer_count_relocations": ["0x88", "0xfb", "0x171"],
            "keep_choice_branch_unchanged": True,
            "availability_route": "stock mobile-parity eligibility retained",
        },
        "achievement": {
        ...
        "control_flow": {
            "find_code_cave": False,
            "was_item_spawned_code_cave": False,
            "handle_mouse_code_cave": True,
            "page_count_code_cave": True,
            "drop_incomplete_reentry_guard": True,
            "drop_first_copy_route": True,
            "achievement_completion_idempotent": True,
        },
    }


def remove_holiday_ornament_collection_art(manifest):
    image_root = OUT / "Images"
    collection_root = image_root / "CollectionOrnaments"
    targets = [
        image_root / HOLIDAY_ORNAMENT_BACKGROUND_FILENAME,
        *[
            collection_root / runtime_name
            for runtime_name, _source_name, _placeholder_name
            in HOLIDAY_ORNAMENT_COLLECTION_FILES
        ],
    ]
    removed = []
    for target in targets:
        if target.is_file():
            target.unlink()
            removed.append(
                str(target.relative_to(OUT)).replace(chr(92), "/")
            )
    if collection_root.is_dir() and not any(collection_root.iterdir()):
        collection_root.rmdir()
    manifest["holiday_ornament_collection_art"] = {
        "enabled": False,
        "status": "removed from Holiday-disabled runtime payload",
        "removed": removed,
        "reason": (
            "No image descriptors reference Holiday Ornament collection art "
            "while the optional collection page is disabled."
        ),
    }
...
def sync_holiday_ornament_collection_art(manifest):
    ...
    image_base = holiday_ornament_collection_image_base(holiday_body_descriptor_count() if ENABLE_HOLIDAY_BODY_TYPES else 0)
    ...
    status = {
        "canonical_source_dir": str(HOLIDAY_ORNAMENT_PREEXTRACTED_ART_DIR),
        "canonical_asset_manifest": str(
            HOLIDAY_ORNAMENT_PREEXTRACTED_ART_DIR / "asset-manifest.json"
        ),
        ...
        "background_image_id": hex(holiday_ornament_collection_background_image_id(holiday_body_descriptor_count() if ENABLE_HOLIDAY_BODY_TYPES else 0)),
        "entries": [],
    }
    ...
    canonical_names = [
        ...
        for runtime_name, _source_name, _placeholder_name
        in HOLIDAY_ORNAMENT_COLLECTION_FILES
    ] + [HOLIDAY_ORNAMENT_BACKGROUND_FILENAME]
    missing_canonical = [
        name
        for name in canonical_names
        if not (HOLIDAY_ORNAMENT_PREEXTRACTED_ART_DIR / name).is_file()
    ]
    if not canonical_manifest_path.is_file():
        missing_canonical.append("asset-manifest.json")
    if missing_canonical:
        raise RuntimeError(
            "Canonical Holiday Ornament artwork is incomplete: "
            + ", ".join(sorted(set(missing_canonical)))
        )
    ...
    canonical_records = {
        row["filename"]: row
        for row in canonical_manifest.get("assets", [])
        if isinstance(row, dict) and "filename" in row
    }
    if set(canonical_records) != set(canonical_names):
        raise RuntimeError(
            "Canonical Holiday Ornament asset manifest does not describe "
            "exactly the 12 icons and collection background"
        )

    verified_canonical = {}
    for name in canonical_names:
        source = HOLIDAY_ORNAMENT_PREEXTRACTED_ART_DIR / name
        record = canonical_records[name]
        digest = hashlib.sha256(source.read_bytes()).hexdigest()
        if digest != record.get("sha256"):
            raise RuntimeError(
                f"Canonical Holiday Ornament artwork hash mismatch: {name}"
            )
        with Image.open(source) as opened:
            image = opened.convert("RGBA")
        if list(image.size) != record.get("dimensions"):
            raise RuntimeError(
                f"Canonical Holiday Ornament artwork size mismatch: {name}"
            )
        if image.getbbox() is None or image.getchannel("A").getbbox() is None:
            raise RuntimeError(
                f"Canonical Holiday Ornament artwork is blank: {name}"
            )
        verified_canonical[name] = {
            "sha256": digest,
            "dimensions": list(image.size),
        }

    background_source = (
        HOLIDAY_ORNAMENT_PREEXTRACTED_ART_DIR
        / HOLIDAY_ORNAMENT_BACKGROUND_FILENAME
    )
    background_target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(background_source, background_target)
    status["background"] = {
        "path": str(background_target.relative_to(OUT)).replace(chr(92), "/"),
        "source": str(background_source),
        **verified_canonical[HOLIDAY_ORNAMENT_BACKGROUND_FILENAME],
    }

    for index, (runtime_name, _source_name, _placeholder_name) in enumerate(
        HOLIDAY_ORNAMENT_COLLECTION_FILES
    ):
        source = HOLIDAY_ORNAMENT_PREEXTRACTED_ART_DIR / runtime_name
        ...
        status["entries"].append({
            ...
            "path": str(target.relative_to(OUT / "Images")).replace(chr(92), "/"),
            "source": str(source),
            **verified_canonical[runtime_name],
        })

    status["collectables_small"] = sync_small_collectables_sheet()
    manifest["holiday_ornament_collection_art"] = {
        **status,
        "status": "copied_from_workspace_preextracted_assets",
        "output_root": str(output_root),
        "asset_manifest_schema": canonical_manifest.get("schema_version"),
    ...
    if HOLIDAY_ORNAMENT_SUPPLIED_ART_DIR.exists() and not missing_supplied:
        ...
        for index, (runtime_name, source_name, placeholder_name) in enumerate(HOLIDAY_ORNAMENT_COLLECTION_FILES):
            ...
            status["entries"].append({
                "collectable": hex(HOLIDAY_ORNAMENT_COLLECTABLE_START + index),
                "image_id": hex(holiday_ornament_collection_item_image_id(index, holiday_body_descriptor_count() if ENABLE_HOLIDAY_BODY_TYPES else 0)),
                "path": str(target.relative_to(OUT / "Images")).replace("\\", "/"),
                "source": str(source),
...
def write_holiday_body_draw_helper(manifest):
    ...
    detail_entries = {
        (entry["gender"], entry["body_value"]): entry
        for entry in manifest.get("holiday_detail_body_frames", {}).get("frames", [])
    }
    image_ids = []
    offset_x = []
    offset_y = []
    ...
    for gender in OUTFIT_STORE_GENDERS:
        for body_value in HOLIDAY_BODY_VALUES:
            entry = detail_entries.get((gender, body_value), {})
            detail_image_ids.append(villager_detail_body_image_id(gender, body_value))
            offset = entry.get("offset") or [0, 0]
            detail_offset_x.append(int(offset[0]))
            detail_offset_y.append(int(offset[1]))
...
static const int kDetailFrameImageCount = {len(detail_image_ids)};
...
static const int kDetailImageIds[kDetailFrameImageCount] = {{
{c_array(detail_image_ids)}
}};
...
static const int kDetailOffsetX[kDetailFrameImageCount] = {{
{c_array(detail_offset_x)}
}};
static const int kDetailOffsetY[kDetailFrameImageCount] = {{
{c_array(detail_offset_y)}
}};
...
static int VF2ResolveDetailGender(theGraphicsManager* graphics, ldwImageGrid* stockGrid, int column) {{
    if (!graphics || !stockGrid) return -1;
    if (stockGrid != graphics->GetImageGrid((EImage)22)) return -1;
    if (column == 1) return 0; // female column in highrez_bodies_final2.png.
    if (column == 0) return 1; // male column in highrez_bodies_final2.png.
    return -1;
}}
...
static int VF2ScaleIntOffset(int offset, int scale) {{
    if (scale == 0) return 0;
    return (offset * scale + (scale >= 0 ? 50 : -50)) / 100;
}}

static int VF2ScaleFloatOffset(int offset, float scale) {{
    float value = (float)offset * scale;
...
extern "C" void __cdecl VF2DrawVillagerBodyFrameImpl(
    ...
    int detailGender = VF2ResolveDetailGender(graphics, stockGrid, frame);
    if (window && graphics && detailGender >= 0 && body >= kHolidayBodyFirst && body < kHolidayBodyFirst + kHolidayBodyCount) {{
        int bodyIndex = body - kHolidayBodyFirst;
        int index = detailGender * kHolidayBodyCount + bodyIndex;
        if (index >= 0 && index < kDetailFrameImageCount) {{
            ldwImageGrid* frameGrid = graphics->GetImageGrid((EImage)kDetailImageIds[index]);
            if (frameGrid) {{
                window->DrawScaled(
                    frameGrid,
                    x + VF2ScaleIntOffset(kDetailOffsetX[index], scale),
                    y + VF2ScaleIntOffset(kDetailOffsetY[index], scale),
                    0,
                    0,
                    scale,
                    mirror != 0
        ...
        if (frame >= 0 && frame < kRoleFrameCounts[role]) {{
            ...
            if (index >= 0 && index < kFrameImageCount) {{
                ...
                if (frameGrid) {{
                    window->DrawScaled(
                        frameGrid,
                        x + VF2ScaleIntOffset(kOffsetX[index], scale),
                        y + VF2ScaleIntOffset(kOffsetY[index], scale),
                        0,
                        0,
                        scale,
                        mirror != 0
...
'''
    ...
    manifest["holiday_body_draw_helper"] = {
        ...
        "detail_image_ids": len(detail_image_ids),
        "body_values": list(HOLIDAY_BODY_VALUES),
            ...
            "items": [
                {
                    ...
                    "price": entry["price"],
                    "source": entry["source"],
                    "icon_image_id": hex(outfit_icon_image_id(entry["gender"], entry["body_value"], holiday_desc_count)),
                }
...
""",
    ...
    manifest["outfit_store_helpers"] = {
        ...
        "icon_image_base": hex(outfit_icon_image_base(holiday_body_descriptor_count() if ENABLE_HOLIDAY_BODY_TYPES else 0)),
...
extern "C" int __cdecl VF2RollOlderVillagerMortality(
    ...
    int hazardMillionths = 999999;
    if (effectiveAge <= 317) {
        hazardMillionths =
            kVF2OlderMortalityHazardsMillionths[effectiveAge - 55];
    }
    return ldwGameState::GetRandom(1000000) < hazardMillionths ? 1 : 0;
}
...
def patch_graphics_manager(manifest):
    ...
    if ENABLE_HOLIDAY_BODY_TYPES:
        ...
        for gender in OUTFIT_STORE_GENDERS:
            gender_title = gender.title()
            for body_value in HOLIDAY_BODY_VALUES:
                image_id = villager_detail_body_image_id(gender, body_value)
                entry = detail_entries.get((gender, body_value))
                path = (
                    entry["path"]
                    if entry
                    else f"VillagerDetailBodies/{gender_title}/Body_{body_value:02d}/Frame00.png"
                )
                vals = plain_image_donor[:]
                vals[0] = image_id
                vals[1] = 0
                ...
                img_sec = obj.section(img_sym.section)
                obj.buf[img_sec.raw_ptr + desc_off : img_sec.raw_ptr + desc_off + DESC_SIZE] = struct.pack("<" + "I" * (DESC_SIZE // 4), *vals)
                sym = f"_vf2detailbody_{gender}_{body_value:02d}_png"
                helper_lines.append(f'const char {sym[1:]}[] = "{path}";')
                symidx = obj.append_undefined_symbol(sym)
                obj.append_relocation(img_sym.section, desc_off + 4, symidx)
                holiday_detail_desc_manifest.append({
                    "gender": gender,
                    "body_value": body_value,
                    "image_id": hex(image_id),
                    "path": path,
    ...
    for label, info in VF3_TV_FLOATING_ANIMS.items():
        image_id = vf3_tv_anim_image_id(label, holiday_desc_count)
    ...
    for entry in outfit_store_entries():
        image_id = outfit_icon_image_id(entry["gender"], entry["body_value"], holiday_desc_count)
    ...
    if ENABLE_HOLIDAY_ORNAMENTS:
        for index, (filename, _source_name, _placeholder_name) in enumerate(HOLIDAY_ORNAMENT_COLLECTION_FILES):
            image_id = holiday_ornament_collection_item_image_id(index, holiday_desc_count)
        ...
        ornament_background_image_id = holiday_ornament_collection_background_image_id(holiday_desc_count)
        vals = plain_image_donor[:]
    ...
    manifest["theGraphicsManager"] = {
        ...
        "holiday_body_frame_images": {
            ...
            "image_count": HOLIDAY_BODY_IMAGE_COUNT if ENABLE_HOLIDAY_BODY_TYPES else 0,
            "descriptors": holiday_body_desc_manifest,
        },
        "holiday_detail_body_images": {
            "enabled": ENABLE_HOLIDAY_BODY_TYPES,
            "image_base": hex(villager_detail_body_image_base()) if ENABLE_HOLIDAY_BODY_TYPES else None,
            "image_count": HOLIDAY_DETAIL_BODY_IMAGE_COUNT if ENABLE_HOLIDAY_BODY_TYPES else 0,
            "descriptors": holiday_detail_desc_manifest,
        },
        "vf3_tv_floating_animation_images": {
            "image_base": hex(vf3_tv_anim_image_base(holiday_desc_count)),
        ...
        "holiday_ornament_collection_images": {
            "enabled": ENABLE_HOLIDAY_ORNAMENTS,
            "image_base": hex(holiday_ornament_collection_image_base(holiday_desc_count)) if ENABLE_HOLIDAY_ORNAMENTS else None,
...
def patch_floating_anim_table(manifest):
    ...
    new_entries = []
    for label, info in VF3_TV_FLOATING_ANIMS.items():
        new_entries.append(struct.pack("<IIII", vf3_tv_anim_image_id(label, holiday_desc_count), 18, 1, 0))
        ...
        "private_vf3_tv_entries": [
            {
                "label": label,
                "enum": hex(info["enum"]),
                "image_id": hex(vf3_tv_anim_image_id(label, holiday_desc_count)),
                "runtime_name": VF3_TV_RUNTIME_ANIMATION_NAMES[label],
                "frames": 18,
...
def patch_custom_achievements(manifest):
    ...
    master_collector_target_off = (
        list_sym.value
        + HOLIDAY_ORNAMENT_MASTER_COLLECTOR_ID * ACHIEVEMENT_ROW_SIZE
        + 4
    )
    if struct.unpack_from(
        ...
        raise RuntimeError("Unexpected Master Collector target count")
    struct.pack_into(
        "<I",
        achievement_obj.buf,
        list_sec.raw_ptr + master_collector_target_off,
    ...
    queue_sec = achievement_obj.section(queue_sym.section)
    queue_bound = queue_sym.value + 0x19
    if (
        achievement_obj.buf[
            queue_sec.raw_ptr + queue_bound : queue_sec.raw_ptr + queue_bound + 3
        ]
        != b"\x83\xF8\x5F"
    ):
    ...
    reset_queue_sec = achievement_obj.section(reset_queue_sym.section)
    reset_queue_count = reset_queue_sym.value + 0x11
    expected_reset_queue_count = b"\xB9\x5F\x00\x00\x00"
    if (
        achievement_obj.buf[
            reset_queue_sec.raw_ptr
            + reset_queue_count : reset_queue_sec.raw_ptr
            + reset_queue_count
            + len(expected_reset_queue_count)
        ]
        != expected_reset_queue_count
    ):
        raise RuntimeError("Unexpected ResetNotifyQueue count")
    struct.pack_into(
        "<I",
        achievement_obj.buf,
...
def patch_older_villager_mortality(manifest):
    ...
    manifest["OlderVillagerMortality"] = {
        ...
        "enabled_path": (
            "one millionth-resolution roll on exact 20-tick birthdays, then rejoin at +0x3C8"
        ),
        "runtime_flag": {
        ...
        "curve": {
            "main_distribution": (
                "annual intensity 0.00365*n + 0.06*max(0,n-55), where "
                "n=max(0, displayed age-(55+active food groups))"
            ),
            "nutrition": (
                "stock threshold T=55+active food groups, clamped to 0-4 groups"
            ),
            "rare_tail": (
                "additional intensity begins after effective age 110; the "
                "999999/1000000 cap retains one non-death outcome"
            ),
            "hard_maximum": None,
            "annual_hazard_cap_millionths": OLDER_MORTALITY_HAZARD_CAP_MILLIONTHS,
            "source": "B155.5 full-game calibrated monotonic curve",
            "random_limit": OLDER_MORTALITY_RANDOM_LIMIT,
...
def patch_collectable_item_holiday_ornaments(manifest):
    ...
    patches.append({
        ...
        "duplicate_route": (
            "Duplicate ornaments retain the stock coin-only path; only a "
            "first copy increments Ornamentologist 0x5F"
        ),
    })
    ...
    manifest["CollectableItemHolidayOrnaments"] = {
        "status": "patched",
        "collectable_range": f"{hex(HOLIDAY_ORNAMENT_COLLECTABLE_START)}-{hex(HOLIDAY_ORNAMENT_COLLECTABLE_END)}",
        "spawn_model": "registered as three exact mobile AddSpawnArea regions; CCollectableItem::Update/Add and Lucky Rock odds remain stock",
        "spawn_area_count": len(HOLIDAY_ORNAMENT_SPAWN_RECTS),
        "mobile_spawn_rects": [
            [hex(value) for value in rect]
            for _symbol, rect in HOLIDAY_ORNAMENT_SPAWN_RECTS
        ...
        "exact_item_matching": {
            "find": "stock exact ECarrying equality retained",
            "was_item_spawned": "stock exact ECarrying equality retained",
            "reason": "mobile 1.7.16 Add selects a concrete 0x9E-0xA9 variant before duplicate checking",
        },
        "patches": patches,
    }
...
def patch_collection_scene_holiday_ornaments(manifest):
    ...
    page_entries = [
        (
            holiday_ornament_collection_item_image_id(index, holiday_desc_count),
            x,
            y,
        )
    ...
    obj.insert_section_bytes(
        ...
        struct.pack("<III", holiday_ornament_collection_background_image_id(holiday_desc_count), 0, 0),
    )
    ...
    draw_expected = b"\x8B\x47\x14\xFF\x74\x87\x18"
    draw_continue = draw_sym.value + 0x184
    draw_cave = draw_sec.raw_size
    draw_payload = bytearray()
    draw_payload += b"\xFF\x77\x14"                  # push [edi+14h] (page)
    draw_payload += b"\xE8\x00\x00\x00\x00"       # call page-count helper
    draw_payload += b"\x50"                           # push returned count
    draw_payload += b"\xE9" + section_rel32(
        draw_cave + len(draw_payload),
        5,
        draw_continue,
    )
    obj.insert_section_bytes(draw_sym.section, draw_cave, bytes(draw_payload))
    ...
    manifest["CollectionSceneHolidayOrnaments"] = {
        ...
        "page_count_helper": "_VF2CollectionPageCount@4",
        "page_count_route": "fixed-size DrawScene detour calls the stdcall page-count helper instead of reading the five-entry stock count cache.",
        "page_count_detour_offset": "0x17d",
        "page_count_code_cave_offset": hex(draw_cave),
        "main_scene_total": {
...
def patch_the_collector_holiday_ornaments(manifest):
    ...
    canfire_sec = obj.section(canfire_sym.section)
    aggregate_helper_name = (
        "?CollectionCountWithHolidayOrnaments@CCollectableItem@@"
        "QBE?BHW4ECarrying@@_N11@Z"
    )
    aggregate_helper = obj.append_undefined_symbol(aggregate_helper_name)
    offer_count_relocations = []
    for operand_off, rarity in (
        (0x088, "common"),
        (0x0FB, "uncommon"),
        (0x171, "rare"),
    ):
        call_off = canfire_sym.value + operand_off - 1
        canfire_sec = obj.section(canfire_sym.section)
        if (
            obj.buf[
                canfire_sec.raw_ptr + call_off : canfire_sec.raw_ptr + call_off + 5
            ]
            != b"\xE8\x00\x00\x00\x00"
        ):
            raise RuntimeError(
                "Unexpected CEventTheCollector::CanFire collection-count call "
                f"at {operand_off:#x}"
            )
        obj.retarget_relocation(
            canfire_sym.section,
        ...
        offer_count_relocations.append({
            "operand_offset": hex(operand_off),
            "rarity": rarity,
            "route": aggregate_helper_name,
        })

    impact_sym = obj.symbol("?ImpactGame@CEventTheCollector@@UAEXH@Z")
    impact_sec = obj.section(impact_sym.section)
    reset_tail = impact_sym.value + 0x5D
    expected_tail = (
    ...
    if (
        obj.buf[
            impact_sec.raw_ptr + reset_tail :
            impact_sec.raw_ptr + reset_tail + len(expected_tail)
        ]
        != expected_tail
    ):
    ...
    reset_helper = obj.append_undefined_symbol(reset_helper_name)
    obj.retarget_relocation(
        impact_sym.section,
        impact_sym.value + 0x6B,
        reset_helper,
    ...
    manifest["TheCollectorHolidayOrnaments"] = {
        ...
        "offer_count_relocations": offer_count_relocations,
        "availability_route": (
            "mobile-parity stock eligibility retained; Holiday counts join all "
            "three offer pools after the event fires"
        ),
        "impact_function": "?ImpactGame@CEventTheCollector@@UAEXH@Z",
        "achievement_reset_route": {
            "tail_relocation_operand": "0x6b",
            "helper": reset_helper_name,
            "stock_tail_argument": "0x5e",
            "holiday_progress": hex(HOLIDAY_ORNAMENT_ACHIEVEMENT_ID),
        },
        "keep_choice_branch": {
            "offset": "0x7",
            "bytes": "75 66",
        ...
        "achievement_reset": hex(HOLIDAY_ORNAMENT_ACHIEVEMENT_ID),
        "note": "Adds ornament base 0x9E to all three offer counts and resets unfinished Ornamentologist progress on Sell without inserting or shifting native branches.",
    }
...
def sync_generation_lock_art(manifest):
    ...
    source_paths = generation_lock_source_paths()
    missing = [str(path) for path in source_paths.values() if not path.is_file()]
    status = {
        "source_mode": "individual_generation_lock_frames",
        "source_dir": str(GENERATION_LOCK_ART_DIR),
        "destination": str(dst),
    ...
    if missing:
        ...
        raise FileNotFoundError(
            "Missing bundled generation lock art: " + ", ".join(missing[:5])
    ...
    strip = Image.new("RGBA", (source_strip_width, LOCKED_GENERATION_CELL_HEIGHT), (0, 0, 0, 0))
    icons = []
    normalized = []
    for frame, generation in enumerate(range(2, LOCKED_GENERATION_FRAME_COUNT + 2)):
        source = source_paths[generation]
        image = Image.open(source).convert("RGBA")
        source_size = list(image.size)
        if image.size != (LOCKED_GENERATION_CELL_WIDTH, LOCKED_GENERATION_CELL_HEIGHT):
            image = image.resize((LOCKED_GENERATION_CELL_WIDTH, LOCKED_GENERATION_CELL_HEIGHT), Image.Resampling.LANCZOS)
            normalized.append(generation)
        icon_path = icon_dir / f"lock_{generation:02d}.png"
        image.save(icon_path)
        strip.paste(image, (frame * LOCKED_GENERATION_CELL_WIDTH, 0))
        icons.append({
            "generation": generation,
            "source": str(source),
    ...
    status.update({
        "copied": True,
        "normalized_generations": normalized,
        ...
        if check_files:
            ...
            if not runtime_path.is_file():
                ...
                try:
                    ...
                    with Image.open(runtime_path).convert("RGBA") as runtime_image:
                        if runtime_image.width % 6 != 0 or runtime_image.height % 3 != 0:
                            errors.append(
                                f"{label} runtime animation sheet is not divisible by 6x3: {runtime_image.size}"
                            )
                        if not runtime_image.getchannel("A").getbbox():
                            errors.append(f"{label} runtime animation sheet is fully transparent: {runtime_path}")
                except Exception as exc:
                    errors.append(f"{label} runtime animation sheet could not be audited: {runtime_path}: {exc}")
...
def validate_invisible_hammock_behavior_contract(manifest):
    ...
    alias = click_aliases.get("0x30c")
    if alias is None:
        errors.append("missing Invisible Hammock donor click alias")
    elif alias.get("donor_item") != "0x1e1":
...
def main():
    ...
    manifest = {
        ...
        "generation_lock_distribution": GENERATION_LOCK_ASSIGNMENT_SUMMARY,
        "items": [
            {
                "name": name,
    ...
    if ENABLE_HOLIDAY_ORNAMENTS:
        ...
        manifest["HolidayOrnamentsCollection"] = {
            "enabled": False,
            "status": "optional patch not selected",
            "reason": "The matching stock-collection executable intentionally retains the vanilla five-page Collections Chest.",
            "stock_collection_total": 60,
            "mobile_collection_total": 72,
            "collectable_range": f"{hex(HOLIDAY_ORNAMENT_COLLECTABLE_START)}-{hex(HOLIDAY_ORNAMENT_COLLECTABLE_END)}",
            "selection": "Enable Holiday Ornaments in the offline patcher to install the fully linked six-page executable and artwork.",
        }
    if ENABLE_BEHAVIOR_PATCHES:
    ...
    if ENABLE_HOLIDAY_BODY_TYPES:
        ...
        sync_holiday_detail_body_frames(manifest)
    else:
        sync_original_villager_sprite_sheets(manifest)
        manifest["holiday_detail_body_frames"] = {
            "status": "disabled; stock highrez_bodies_final2.png rows 0-49 retained",
            "body_values": list(HOLIDAY_BODY_VALUES),
        }
