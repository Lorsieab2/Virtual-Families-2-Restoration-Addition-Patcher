...
try {
    for ($mask = $StartMask; $mask -lt 16; ++$mask) {
        ...
        $previous = Join-Path $outputs "VF2-Mobile-Furniture-With-Island-Events-B152-$variant"
        $out = Join-Path $outputs "VF2-Mobile-Furniture-With-Island-Events-B153-$variant"
...
Write-Host "Running B153 linked Holiday positive/negative validation"
& $pythonExe @pythonArgs $holidayValidator
if ($LASTEXITCODE -ne 0) {
    throw "B153 linked Holiday validation failed with exit code $LASTEXITCODE"
}

Write-Host "Running B153 linked runtime-flag validation"
$runtimeReport = Join-Path $outputs "B153-Runtime-Flag-Validation.json"
& $pythonExe @pythonArgs $runtimeValidator --output $runtimeReport
if ($LASTEXITCODE -ne 0) {
    throw "B153 linked runtime-flag validation failed with exit code $LASTEXITCODE"
}

Write-Host "B153 16-state executable matrix and linked validation completed successfully."
