...
try {
    for ($mask = $StartMask; $mask -lt 16; ++$mask) {
        ...
        $patchLog = Join-Path $out "b155-patch.log"
        $buildLog = Join-Path $out "b155-build.log"
        ...
        $debuggerReport = Join-Path $out "b155-debugger-validation.json"
...
Write-Host "Running B155 linked runtime-flag validation"
$runtimeReport = Join-Path $outputs "B155-Runtime-Flag-Validation.json"
& $pythonExe @pythonArgs $runtimeValidator --build-label B155 --output $runtimeReport
if ($LASTEXITCODE -ne 0) {
    throw "B155 linked runtime-flag validation failed with exit code $LASTEXITCODE"
}

Write-Host "B155 16-state executable matrix and linked validation completed successfully."
