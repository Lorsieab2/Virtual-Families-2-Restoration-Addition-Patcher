...
def validate_tooltip_pe(image: PEImage, scene: dict[str, Any]) -> None:
    ...
    tooltip_base = TOOLTIP_LABELS[0] - 15
    tooltip_pattern = b"\x8D\x98" + struct.pack("<I", tooltip_base)
    require(
        tooltip_pattern in cave_data,
        f"tooltip cave no longer maps buckets 15-17 to labels {TOOLTIP_LABELS!r}",
    )
    require(
...
def validate_positive_achievement_pe(
    ...
    require(order.get("adjacent") is True, "Ornamentologist must immediately follow Bottlologist")
    require(as_int(order.get("bottlologist_id"), "Bottlologist ID") == 0x5E, "Bottlologist ID drifted")
    require(as_int(order.get("ornamentologist_id"), "Ornamentologist order ID") == 0x5F, "Ornamentologist order ID drifted")

    queue = custom.get("notification_queue")
    require(isinstance(queue, dict), "achievement notification queue contract missing")
    require(as_int(queue.get("capacity_dwords"), "notification queue capacity") == 0x5F, "notification queue capacity must remain 95 dwords")
    require(as_int(queue.get("pop_shift_dwords"), "notification queue shift") == 0x5E, "notification queue shift must remain 94 dwords")
...
def validate_negative_manifest(
    ...
    achievement = nested(manifest, "HolidayOrnamentAchievement")
    require(as_int(achievement.get("achievement_id"), "disabled Ornamentologist ID") == 0x5F, "disabled Ornamentologist ID drifted")
    require(as_int(achievement.get("master_collector_target"), "disabled Master Collector target") == 5, "disabled Master Collector target must remain 5")
    require(as_int(achievement.get("goal_collector_target"), "disabled Goal Collector target") == 12, "disabled Goal Collector target must remain 12")
    require(as_int(achievement.get("notification_queue_count"), "disabled notification count") == 0x5F, "disabled notification count must remain 0x5F")
...
def validate_negative_pe(image: PEImage, source: SourceContract) -> None:
    ...
    # B153 links the dormant Ornamentologist achievement row into every
    # executable so enabling Holiday Ornaments does not require table growth.
    # Only collection runtime assets must remain absent while the patch is off.
    for text in (
