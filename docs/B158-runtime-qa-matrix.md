...
| `R-03` Renovation direct removal/rebuy | `P2` plus `cheat_upgrades=on`; own an active cosmetic style and a previously purchased inactive style; invoke the Cheat Upgrades-gated removal path, then rebuy the old style and buy/switch another style. | Removal clears only the active room style, preserves purchase history, returns the stock room, saves immediately, and displays the explicit catalog price for an inactive rebuy; no structural renovation record is deleted. | Reload after removal, after rebuy, and after a new purchase; compare room, active byte, history mask, price, and save identity. | `P3` leaves the stock renderer and native renovation inventory untouched; no mobile style survives in the disabled runtime tree. | Capture any crash during direct removal, immediate save, or rebuy with item ID and state bytes. |
