...
| `P1` | Mobile furniture enabled: offline patcher setting `mobile_furniture_behaviors`; retain the matching `.vf2beh` executable and optional map payload. | Setting state, EXE identity, presence/hash of each route's `.fmap`/asset, clean-save backup. |
