...
| Mobile Island Events and exact outcomes | Partial | Added event objects/text remain optional. Exact disassembly now preserves dummied-out `MeteoriteFallsInYard1`, the `StrangePackageOnPorch` 50-149 coin choice, and the `Teens` 10-sock/10-trash or -75-coin choice with its raw-age 260-340 target gate. Remaining native outcomes/state changes still require one-by-one audits. |
