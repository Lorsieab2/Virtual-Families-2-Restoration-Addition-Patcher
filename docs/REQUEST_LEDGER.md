...
| Mobile-only furniture actions | B156 lounge family player-verified / remaining routes pending | Scope is exactly the 63 genuine mobile rows `0x2AA-0x2E8`; all Invisible/custom/VF3 items are excluded. Forty-one original QAMFs are tracked locally and 22 absent maps are explicit. Player testing confirms the Lounge Chair anchor/pose and bad-weather refusal. Good-weather manual drops now randomly choose relaxing, reading, studying, sitting, napping, or sleeping; nap and sleep use documented weights from native energy at `CVillager+0x6B28`. Autonomous routes include mobile-derived reading/napping/daytime sunbathing plus sitting and the identified studying extension. Patio Umbrella has its exact manual plan route. Exact disable restoration is automated; live random-choice frequency QA remains. Holiday, birthday, other patio, and picnic families remain audited but unported until their exact routes and anchors are proven. Disruptive group actions must remain manual-drop-only. |
