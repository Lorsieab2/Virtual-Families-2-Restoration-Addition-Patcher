...
| Mobile-only furniture actions | B156 lounge family player-verified; Umbrella, Birthday Cake, Birthday Presents, and Christmas Stockings source complete / remaining routes pending | Scope is exactly the 63 genuine mobile rows `0x2AA-0x2E8`; all Invisible/custom/VF3 items are excluded. Forty-one original QAMFs are tracked locally and 22 absent maps are explicit. Player testing confirms the Lounge Chair anchor/pose and bad-weather refusal. Good-weather manual drops now randomly choose relaxing, reading, studying, sitting, napping, or sleeping; nap and sleep use documented weights from native energy at `CVillager+0x6B28`. Autonomous routes include mobile-derived reading/napping/daytime sunbathing plus sitting and the identified studying extension. Patio Umbrella has its exact manual plan route. Child-only Birthday Cake and Birthday Presents, plus the under-18 Large and Small Christmas Stockings action, now have exact direct plan ports with minimal PC-safe maps. Birthday Balloons is fully audited and is next; Banner/group, whole-family Holiday routes, other patio, and picnic routes remain unported until each complete route is safe. Exact disable restoration is automated; live birthday, stocking, and random-choice QA remain. |
