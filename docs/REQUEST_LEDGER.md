...
| Mobile-only furniture actions | B156 lounge family player-verified; Patio Table, Umbrella, Birthday Cake, Birthday Presents, Birthday Balloons, Birthday Banner, Christmas Stockings, Dreidel, and Menorah source complete / remaining routes pending | Scope is exactly the 63 genuine mobile rows `0x2AA-0x2E8`; all Invisible/custom/VF3 items are excluded. Forty-one original QAMFs are tracked locally and 22 absent maps are explicit. Player testing confirms the Lounge Chair anchor/pose and bad-weather refusal. Good-weather manual drops now randomly choose relaxing, reading, studying, sitting, napping, or sleeping; nap and sleep use documented weights from native energy at `CVillager+0x6B28`. Autonomous routes include mobile-derived reading/napping/daytime sunbathing plus sitting and the identified studying extension. Patio Umbrella has its exact manual plan route. Patio Table now has exact guarded manual Preparing Drinks and Drink at Patio Chair routes, two preserved chair anchors, and a PC-safe external 240-game-second drinks state; children may drink once ready, while preparation requires adult raw age `0x118+` and at least 31 food. Its mobile autonomous pair `0x1B6/0x1B7` remains deliberately unindexed because no regression-safe PC surrogate pair is proven, and save/reload persistence for the short external timer remains unproven. The complete Birthday family includes guarded single-object fallbacks and Birthday Banner whole-household celebration. Large and Small Christmas Stockings, Dreidel, and Menorah also have exact guarded plan ports with minimal PC-safe maps. Other Holiday and picnic routes remain unported until each complete route is safe. Exact disable restoration is automated; live Patio, birthday, Holiday, and random-choice QA remain. |
