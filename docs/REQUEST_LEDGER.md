...
| Mobile Island Events and exact outcomes | Partial | Added event objects/text remain optional. Thirteen events now follow exact mobile firing/outcome evidence. `EmailFromSchool` includes exact parent/child selection, behavior, award, and state byte; `InterestingArticleAboutFossils` includes exact behavior, happiness, fossil-family, and coordinate rolls. Other outcomes still require one-by-one audits. |
