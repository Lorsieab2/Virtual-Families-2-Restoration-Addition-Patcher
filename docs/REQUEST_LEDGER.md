...
| Mobile-only furniture actions | B156 first exact family implemented / remaining routes pending | Scope is exactly the 63 genuine mobile rows `0x2AA-0x2E8`; all Invisible/custom/VF3 items are excluded. Forty-one original QAMFs are tracked locally and 22 absent maps are explicit. Lounge Chairs `0x2DE-0x2E1` now use a default-off `.vf2beh` dispatcher, exact `LieOnChaiseNoLeadIn` plan sequence, and four PC-safe placed-item maps with exact disable restoration. Holiday, birthday, patio, umbrella, and picnic families remain audited but unported until their exact routes and anchors are proven. Disruptive group actions must remain manual-drop-only. |
