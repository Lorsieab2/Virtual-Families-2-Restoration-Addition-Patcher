...
| Request | Status | Completion contract |
|---|---|---|
| Mobile Kitchen/Office/Workshop/first Bathroom renovations | B156 art staged; native map compositing not yet implemented | Fifteen upright, hash-verified mobile room PNGs are local under `work/assets/mobile_renovations/` and staged into the optional payload. Mobile `0xE1-0xEA` load/activation records are documented in `docs/B156-mobile-renovation-ledger.md`; the PC renderer is confirmed as `CWorldMap::Draw` over 16 `MapX##Y##.jpg` tiles, but exact overlay scale/anchors, renovation-state selection, store bindings, saved choice, switching/removal, and off-state remain pending native evidence. |
