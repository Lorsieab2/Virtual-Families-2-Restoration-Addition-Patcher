...
- In-game Cheat Upgrades test B131: buy `Unlock all furniture` and confirm
  ...
  store rows.
- Patcher smoke B135: extract the B135 patcher ZIP, run Dry Run with all Main
  ...
  and the optional song restore path now that source-only payload folders are
  copied into the portable patcher.
