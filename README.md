...
- Current playtest follow-up: the Exercise Bike and Home Gym venue handlers now apply their own labels even when the borrowed donor keeps its native label, while preserving the donor animation and duration. Spa receiving now uses the stock SleepNW/SleepNE strips after a short settle and walks to a small upward-adjusted target. **Source/test evidence only until a generated build and player QA confirm the result.**
