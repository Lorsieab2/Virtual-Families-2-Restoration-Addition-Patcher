...
- **FIXED - this setting used to prevent the game from starting.** With **Add mobile furniture behaviors** enabled, the game faulted a few seconds after launch, before reaching play. Turning the setting off was a complete workaround, and on the affected releases it still is: every other feature is unaffected and the rest of the patcher works normally. It was never a regression between releases -- it reproduced on B179, B180 and B181 alike, and the one-byte `.vf2beh` runtime flag was both necessary and sufficient to reproduce or avoid it. The cause was a skipped register restore: an injected branch jumped into a stock epilogue past the instruction that restores a saved register, so the following restores took the wrong stack slots and the villager pointer ended up in the wrong register. The fix adds that restore on the injected path. It is confirmed in the built game rather than only in source -- the corrected executable, with the setting enabled, runs without faulting, while an unfixed build in the same install still faults within seconds. If you are running an earlier release the workaround above still applies. See `docs/Transparency Log.txt` for the measurements.
