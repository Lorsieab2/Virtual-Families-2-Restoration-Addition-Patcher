...
- **FIXED IN SOURCE, NOT YET IN ANY RELEASE - this setting stops every published build from starting, B181 included.** With **Add mobile furniture behaviors** enabled, a published build faults a few seconds after launch, before reaching play. Turning the setting off is a complete workaround on those builds: every other feature is unaffected and the rest of the patcher works normally. This is not a regression between releases -- it reproduces on B179, B180 and B181 alike, and the one-byte `.vf2beh` runtime flag is both necessary and sufficient to reproduce or avoid it. The cause is a four-byte stack displacement: an injected branch jumps into a stock epilogue past the instruction that restores a saved register, so the following restores take the wrong stack slots and the villager pointer ends up in the wrong register. THIS AFFECTS EVERY RELEASE PUBLISHED SO FAR, INCLUDING B181. The cause has been found and repaired in source, and a game built from the current source with the setting enabled starts and runs -- verified against an unfixed build that still faults -- but that repair is not in any published archive yet. Until a fixed release is published, turn the setting off. See `docs/Transparency Log.txt` for the measurements.
