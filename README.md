...
- **Cheat Upgrades** (on) - the cheat-only executable overlay: money, food, achievement/puzzle/collection, price, and malfunction rows, including Trigger and Fix all house malfunctions; the wellbeing rows (Clear All Illnesses and Max out Happiness / Energy / Fed / Health Bar), which apply to everyone currently in the house; and the pregnancy one-shots (Force Successful Pregnancy, Next Babies Male/Female, Next Pregnancy Singleton/Twins/Triplets). Buying an armed one-shot again cancels it, an armed row shows a checkmark, and arming one clears the rows it is mutually exclusive with. Two more rows toggle the ownership of stock Flea Market upgrades -- Anti-Spam Software and the Rockhound Certificate -- without needing to use the item on a computer; the Flea Market rows themselves are untouched base game.
