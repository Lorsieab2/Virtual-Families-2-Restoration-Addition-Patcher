...
- **Four new visible furniture items** - Exercise Bike, Home Gym System, Ping-Pong Table, and Spa Lounger. These are ordinary store items with their own art, each built on the same donor arrangement as the invisible pieces above. Until B180 none of them did anything when a villager was dropped on one: this patcher's drop dispatcher matches on item id, and only the Invisible Spa Lounger was ever listed, so every other added piece had no route at all. Their records and placement maps were correct and simply never consulted. B180 gives the **Spa Lounger** a route of its own. The **Exercise Bike**, **Home Gym System** and **Ping-Pong Table** instead rely on the game's own hotspot path, through the stock item each was modelled on. Two things about those three have since been reported from live play and are worth stating plainly. First, the **Home Gym System** does not do anything: its donor is the Yoga Equipment, and the yoga behaviours are in-place animations that never consult a piece of furniture at all, so cloning that donor produced a faithful clone of a decoration. That is not fixed yet. Second, the **Ping-Pong Table**'s label was wrong for a separate reason, described under "Labels for the added furniture" below, and that one is fixed.
